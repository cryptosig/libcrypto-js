...
(function(root, factory) {
  if (typeof exports === 'object' && typeof exports.nodeName !== 'string') {
    ...
    // Browser globals
    root.peerplaysio = root.peerplaysio || {};
    factory((root.peerplaysio.crypto = {}));
  }
  ...
  function PrivateKey(priv, pub) {
    ...
    this.sign = function(hash) {
      return fromBits(sjcl.codec.peerplaysio.signRecoverably(priv, toBits(hash)));
    };
  }

  PrivateKey.from = function(wif, header) {
    return new PrivateKey(
      sjcl.codec.peerplaysio.deserializePrivateKey(wif, header)
    );
  };
  ...
  PublicKey.from = function(str) {
    return new PublicKey(sjcl.codec.peerplaysio.deserializePublicKey(str));
  };

  PublicKey.recover = function(hash, sig) {
    return new PublicKey(
      sjcl.codec.peerplaysio.recoverPublicKey(toBits(hash), toBits(sig))
    );
  };

  PublicKey.prototype = {
    toString: function() {
      return sjcl.codec.peerplaysio.serializePublicKey(this._p);
    },
  ...
  function serializePair(k) {
    return {
      private: sjcl.codec.peerplaysio.serializePrivateKey(k.sec),
      public: sjcl.codec.peerplaysio.serializePublicKey(k.pub)
    };
  }
